...
// Private variables
static volatile adc_boost_config config;
...
static THD_FUNCTION(adc_thread, arg) {
	...
	for(;;) {
		...
		//limit throttle if held for a too long period
		...
		// boost_meter slowly cools down if current is lower than boost_start
		if (fabsf(pwr) < config.boost_start) {
			boost_meter += (pwr- config.boost_start) / config.boost_cooldown_factor;
		}

		if (boost_meter > config.max_boost_time* config.update_rate_hz* (1.0 - config.boost_start) ) {
			boost_enabled = false;
		}
		...
		//limit the power if boost_meter is full
		if (boost_enabled == false){
			utils_truncate_number(&pwr, -config.boost_start, config.boost_start);
		}


		// Apply ramping
		static systime_t last_time = 0; //static means the variable will be initialized to 0, but keep its value in the next loop iterations
		...
		if (ramp_time > 0.01) {
			...
			pwr = pwr_ramp; // otherwise power will just keep its old value
		}
